jQuery(document).ready(function($){
    ...
    function renderResults(data) {
        ...
        if (!data.postsData || data.postsData.length === 0) {
            $("#myLoaderGif").toggle();
            html = '<p><strong>No es troben posts per ' + $('#startUrl').val() + '.</strong></p>';
        ...
        //html += '<p><button id="garbell-cancel" type="button" class="button button-primary">Cancelar</button></p>';
        ...
        data.postsData.forEach(function(post, idx){
            ...
            html += '<td style="width: 10%;">SI <input type="checkbox" class="g-select" data-idx="'+idx+'">';
        ...
        $('#garbell-import').on('click', function(){
           ...
           //mostrem el gif de carga en començar la importació
           // $("#myLoaderGifImport").toggle();
            ...
            if (payload.length === 0) {
                ...
                */
            /**************************************
             * pasar payload a contributor-page.php
             * en garbell_ajax_import_selected()
             **************************************/
            ...
            $.post(garbell_ajax.ajax_url, send, function(resp){
                ...
                //$("#myLoaderGifImport").toggle();
                if(isEmpty(JSON.stringify(resp.data.inserted_post_ids))){
        ...
        /*
        $('#garbell-cancel').on('click', function(){
            $('#garbell-import').hide();
            $('#garbell-cancel').hide();
            $('#garbell-table').hide();
            $('#garbell-run').show();
