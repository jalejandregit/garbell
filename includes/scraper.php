...
*/
    ...
    private function crawl($url, $depth) {
        ...
        foreach ($linkNodes as $a) {
            ...
            /*
            if (!empty($this->tagsExclusions)) {
                $patterns = is_array($this->tagsExclusions)
                    ? $this->tagsExclusions
                    : preg_split('/\|/', $this->tagsExclusions);
    ...
    private function extract_post_data($url) {
        ...
        */
        ...
        $querySelectorImages ="//*[@id='main']//a[img]";
        if (!empty($this->selectorImages)) {
            try {
                $querySelectorImages = str_replace('\\', '', $this->selectorImages);
                //$querySelectorImages = $this->selectorImages;
                //if ($nodesImg && $anchorImgs->length > 0) {
                    //$isPost = true;
                //}
            } catch (Exception $e) {
                // ignore selector parse errors
        ...
        $anchorImgs = $xpath->query($querySelectorImages);

        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("garbell_Scraper __construct opts: " . print_r($querySelectorImages, true));
        }
        ...
        foreach ($anchorImgs as $a) {
            ...
            if($this->tags_exclusions_url($href)) continue;
    ...
    private function tags_exclusions_url($url) {
        if (empty($this->tagsExclusions)) return false;
        $patterns = is_array($this->tagsExclusions)
            ? $this->tagsExclusions
            : preg_split('/\|/', $this->tagsExclusions);
